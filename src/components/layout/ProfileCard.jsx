...
const ProfileCard = ({ isCollapsed, setCollapsed }) => {
  ...
  const handleSaveGoal = () => {
    if (tempWeeklyGoal < 1 || tempWeeklyGoal > 300) {
      toast({
        ...
        description: "Haftalık hedef 1 ile 300 arasında olmalıdır.",
        variant: "destructive",
      });
      return;
    }
    ...
    toast({
      ...
      description: "Yeni hedefin başarıyla kaydedildi.",
      variant: "success",
    });
  };
  ...
  const displayName =
    user?.name || profile?.name || user?.email?.split("@")[0] || "Gezgin";
  const avatarUrl =
    user?.avatar_url ||
  ...
  const weeklyProgress =
    weeklyGoal > 0 ? Math.min((readStoriesCount / weeklyGoal) * 100, 100) : 0;
  ...
  return (
    <aside
      className={cn(
        "fixed right-0 top-0 hidden h-full flex-col border-l bg-background s-desktop:flex transition-all duration-300 ease-in-out",
        ...
        <div className='flex flex-col items-center space-y-4'>
          ...
          <TooltipProvider delayDuration={0}>
            <Tooltip>
              ...
              <TooltipContent side='left' align='center'>
                <p>
                  Haftalık Hedef: {readStoriesCount}/{weeklyGoal}
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        ...
        <div className='flex flex-col h-full'>
          <div>
            ...
            >
              ...
              <p className='mt-1 text-sm text-muted-foreground'>
                Yolculuğuna devam et ve hedeflerine ulaş.
              </p>
            </motion.div>
                ...
                {isEditing ? (
                  ...
                  >
                    <div className='flex items-center justify-between mb-2'>
                      ...
                      >
                        <Goal className='h-4 w-4 text-primary' /> Haftalık Hedef
                      </Label>
                      <Button
                        variant='ghost'
                        size='icon'
                  ...
                  >
                    <div className='flex items-center justify-between mb-2'>
                      <p className='font-semibold text-base flex items-center gap-2'>
                        <Target className='h-4 w-4 text-primary' /> Haftalık
                        İlerleme
                      </p>
                      <Button
                        variant='ghost'
                    ...
                    <div className='flex justify-between text-sm font-medium text-muted-foreground mb-1'>
                      <span>
                        {readStoriesCount} / {weeklyGoal} Hikaye
                      </span>
                ...
                {recentActivities.length > 0 ? (
                  recentActivities.map((activity) => (
                    <div
                      ...
                      onClick={() => navigate(`/story/${activity.slug}`)}
                    >
                      <div className='w-10 h-10 rounded-lg overflow-hidden shrink-0'>
                        <img
                          ...
                          src={
                            activity.image_url ||
                            "https://images.unsplash.com/photo-1695313486156-469b82f59e07"
                          }
                        />
                      </div>
                      <div className='flex-1 overflow-hidden'>
                        ...
                        <div className='flex items-center text-xs text-muted-foreground'>
                          <p>
                            {activity.type === "read" ? "Okundu" : "Kaydedildi"}
                          </p>
                          <span className='mx-1.5'>·</span>
                          <Badge
                            variant='secondary'
                            className={cn(
                              `level-badge level-${activity.level}`
                            )}
                          >
          ...
          <div className='mt-auto pt-6 text-center'>
            <p className='text-xs text-muted-foreground'>
              &copy; {new Date().getFullYear()} HikayeGO
            </p>
          </div>
        </div>
      )}
