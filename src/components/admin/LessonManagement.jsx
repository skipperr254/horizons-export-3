...
import { Plus, Edit, Trash2, Loader2, GripVertical, ChevronDown, ChevronUp, UploadCloud } from 'lucide-react';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
const CategoryForm = ({ level, onCategoryAdded, onUpdateCategoryOrder }) => {
...
const SortableLessonItem = ({ lesson, categoryName, onEdit, onDelete }) => {
    ...
    return (
        <div ref={setNodeRef} style={style} className={cn("flex items-center gap-2 p-2 rounded-md bg-background", isDragging && "shadow-lg")}>
...
const LessonManagement = ({ lessons, categories, onAddLesson, onUpdateLesson, onDeleteLesson, onCategoryAdded, onUpdateLessonOrder }) => {
  ...
  const handleSave = async (lessonData, videoFile) => {
    ...
    delete finalLessonData.video_url;
  ...
  const lessonsByLevelAndCategory = useMemo(() => {
    ...
    levelOptions.forEach(level => {
        structured[level.value] = [];
        const levelCategories = categories.filter(c => c.level === level.value).sort((a, b) => a.position - b.position);
        const levelLessons = activeLessons.filter(l => l.level === level.value);

        levelCategories.forEach(cat => {
            structured[level.value].push({
                ...cat,
        ...
        if (uncategorizedLessons.length > 0) {
            structured[level.value].push({ id: 'uncategorized', name: 'Kategorisiz', lessons: uncategorizedLessons });
        }
    });
    return structured;
  }, [activeLessons, categories]);

  const handleDragEnd = (event) => {
    const { active, over } = event;
    if (active.id !== over.id) {
        const oldIndex = activeLessons.findIndex((l) => l.id === active.id);
        const newIndex = activeLessons.findIndex((l) => l.id === over.id);
        const newOrder = arrayMove(activeLessons, oldIndex, newIndex);
        
        setActiveLessons(newOrder);
        onUpdateLessonOrder(newOrder);
    }
  };
            ...
            {openLevels[level.value] && (
              <CardContent>
                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                  <SortableContext items={activeLessons.filter(l => l.level === level.value).map(l => l.id)} strategy={verticalListSortingStrategy}>
                    <div className="space-y-2">
                      {lessonsByLevelAndCategory[level.value].map(category => (
                        <div key={category.id}>
                          <h3 className="font-semibold text-lg my-2 pl-2">{category.name}</h3>
                          <div className="space-y-2 border-l-2 border-primary/20 pl-4">
                            {category.lessons.map(lesson => (
                              <SortableLessonItem 
                                key={lesson.id} 
                                lesson={lesson} 
                                categoryName={categories.find(c => c.id === lesson.category_id)?.name || 'Kategorisiz'}
                                onEdit={handleEdit}
