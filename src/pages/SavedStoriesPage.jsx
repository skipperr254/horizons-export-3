  ...
  const fetchSavedStories = async () => {
    ...
    try {
      ...
      if (!canAccessPremiumFeatures) {
        ...
        const { unlocked: dailyUnlockedIds } = getDailyA1Stories(allStories || []);
        
        finalSavedStories = finalSavedStories.filter(saved => {
          if (saved.stories?.level === 'a1') {
      ...
      <div className="min-h-screen">
        <div className="container mx-auto py-8 px-4 sm:px-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-8">
            <div className="relative overflow-hidden rounded-2xl p-6 sm:p-8 border border-primary/20 shadow-lg bg-card">
              <div className="relative z-10 flex flex-col sm:flex-row justify-between items-start text-foreground">
                <div className="mb-4 sm:mb-0">
                  ...
                  {!canAccessPremiumFeatures && (
                    <div className="mt-4 flex items-center gap-2 text-sm text-muted-foreground bg-secondary p-3 rounded-lg border max-w-lg">
                      <Info className="h-5 w-5 flex-shrink-0 text-primary" />
                      <span>Ücretsiz planda, yalnızca o gün için erişilebilir olan A1 seviye hikayelerinizi burada görebilirsiniz.</span>
                    </div>
                  )}
                </div>
                <div className="flex-shrink-0 bg-secondary rounded-full w-20 h-20 flex flex-col items-center justify-center border text-center">
                      ...
                      <CardHeader className="p-0">
                        <Link to={storyUrl} className="block">
                          <div className="relative overflow-hidden cursor-pointer aspect-video">
                            <img alt={story.title} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" src={story.image_url || "https://images.unsplash.com/photo-1626447857058-2ba6a8868cb5"} />
