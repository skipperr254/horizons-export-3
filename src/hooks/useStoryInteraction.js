...
export const useStoryInteraction = (user, story, selectedWord, toast, triggerPremiumModal, currentPage, storyContentRef) => {
  ...
  const saveProgress = useCallback((isMarkingPosition = false) => {
    if (storyContentRef.current && user && story) {
  ...
  }, [user, story, currentPage, storyContentRef]);
